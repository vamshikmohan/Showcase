...
def extract_table_excel(path):
    ...
    # --- Handle same Course Code + Name but different slots ---
    grouped = df.groupby(["Course Code", "Course Name"])

    for (code, name), group in grouped:
        unique_slots = group["Approved Time Slot"].nunique()
        if unique_slots > 1:  # only when multiple *different* slots exist
            indices = group.index
            for i, idx in enumerate(indices, start=1):
                df.at[idx, "Course Code"] = f"{code}-{i}"
                df.at[idx, "Course Name"] = f"{name} ({i})"
                df.at[idx, "Display Name"] = f"{df.at[idx, 'Course Code']} - {df.at[idx, 'Course Name']}"

    return df
def set_mandatory(df, mandate_list):
    # Reset first
    df["Mandatory"] = False
    df["Selected"] = df["Selected"] & False  # reset all selections (will reapply below)

    # Mask for mandatory courses
    mask = (
...
def set_selected(df, selected_list):
    # Reset selected (but preserve mandatory later)
    df["Selected"] = df["Mandatory"]

    # Mask for additional selected
    mask = (
...
def slot_to_str(slot):
    """Convert decimal hours (e.g., 11.5) into HH:MM format."""
    h, m = divmod(int(slot[0] * 60), 60)
    h2, m2 = divmod(int(slot[1] * 60), 60)
    return f"{h}:{m:02d}–{h2}:{m2:02d}"

DAY_NAMES = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]

def format_overlap(overlap_list):
    DAY_NAMES_DICT = {1: "Mon", 2: "Tue", 3: "Wed", 4: "Thu", 5: "Fri", 6: "Sat"}

    if not overlap_list:
        return ""
    formatted = []
    for item in overlap_list:
        # item looks like (day_idx, interval1, interval2)
        if isinstance(item, tuple) and len(item) == 3:
            day_idx, slot1, slot2 = item
            day = DAY_NAMES[day_idx] if day_idx < len(DAY_NAMES) else f"Day{day_idx}"
            formatted.append(f"{day}: {slot1[0]:.2f}–{slot1[1]:.2f} overlaps {slot2[0]:.2f}–{slot2[1]:.2f}")
        else:
            formatted.append(str(item))  # fallback for unexpected cases
...
def format_weekly_schedule(schedule):
    ...
    result = []
    for day_idx, slots in enumerate(schedule):  # here day_idx = 0..5
        DAY_NAMES_INDEXED = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
        day_name = DAY_NAMES_INDEXED[day_idx]
        day_slots = []
        for slot in slots:
            if isinstance(slot, (list, tuple)) and len(slot) == 2:
                ...
                start_h, start_m = divmod(int(start * 60), 60)
                end_h, end_m = divmod(int(end * 60), 60)
                day_slots.append(f"{start_h}:{start_m:02d}–{end_h}:{end_m:02d}")
        if day_slots:
            result.append(f"{day_name}: {', '.join(day_slots)}")
